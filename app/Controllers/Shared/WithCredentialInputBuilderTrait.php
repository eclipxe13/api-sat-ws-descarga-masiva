    ...
    private function buildCredential(): Credential
    {
        try {
            ...
            if (! $credential->isFiel()) {
                throw new Exception(
                    sprintf('The certificate %s is not a FIEL', $credential->certificate()->serialNumber()->bytes()),
                );
            }
            if (! $credential->certificate()->validOn()) {
                throw new Exception(
                    sprintf('The certificate %s is expired', $credential->certificate()->serialNumber()->bytes()),
                );
            }
            return $credential;
